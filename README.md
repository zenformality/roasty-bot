...
1. slack app at [api.slack.com/apps](https://api.slack.com/apps), from scratch. enable socket mode, which walks you through making the app-level token (`xapp-`). under oauth & permissions add scopes: `chat:write`, `commands`, `app_mentions:read`, `channels:history`. install to workspace and grab the bot token (`xoxb-`).
2. create those five slash commands with the exact same names, case-sensitive. slack insists on a request url even though socket mode ignores it entirely, put whatever.
